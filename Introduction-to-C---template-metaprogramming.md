...
as C++ clsses and its object-oriented facilities.
...
a function for any type. Metaprogramming in C++ is realizes (almost) entirely
using templates. A template is created using the `template` keyword, followed
by a list of comma separated template arguments, enclosed in _pointy brackets_
...
The exact situations when this can be done are formally described in [link]()
of the standard. A less formal (but possibly not completely accurate)
...
an implicit specialization (and compile the methods of the type) with template
parameters substituted with the arguments passed to the template. As with
function template, a distinct type is created for each unique combination of
...
richer interface than the natively supported C array can be created by using a
type template parameter representing the type of elements stored in the array,
and an integer parameter representing the size of the array:
...
values which are used if an argument is not supplied for that parameters.
...
resource manager how should the resource be released. This can be done by using
...
int main() {
    int x;
    int y;
    decltype(3) z;  // z is of the same type as 3 (i.e. int)
    decltype(2*x + 3.0*y) w; // w is of the same type as the result of
                             // 2*x + 3.0*y (i.e. double)
    decltype(x) x2; // x2 is of the same type as x (i.e. int&)
}
```
...
The answer is obviously "whatever is the return type of `t * u + v`". Thus,
...
parameters `t`, `u` and `v` are used before they are defined.
...
objects used in this solution are rvalues, while the objects in the function
are lvalues.
...
Instead of providing the return type before the function declaration, the
declaration can begin with `auto`, and the return type provided by using the
arrow (`->`) operator and specifying the return type after the function
